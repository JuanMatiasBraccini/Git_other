...
# FUNCTION FOR RUNNING DELTA-X STANDARDISATION METHODS WITH YEAR-CELL INTERACTIONS 
...
#				. Delta indices are computed for several error distributions. 
...
Delta_Log_inter=function(DATA,VARIABLES,FORMULA.bin,FORMULA.pos)
{
 
  # 1. Put data in proper shape
  DataFile=DATA[,match(VARIABLES,names(DATA))]
  DataFile=DataFile[order(DataFile$FINYEAR),]
